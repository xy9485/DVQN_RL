...
def parse_log_files(
    file_name_template, key_name, value_name, num_seeds, smooth, best_k=None, max_key=True
):
    all_values = []
    all_keys = []
    actual_keys = None
    root = "/" + os.path.join(*file_name_template.split("/")[:-2])
...
def print_result(
    ...
    file_name_template = os.path.join(root, "%s", file_name)
...
def print_planet_baseline(task, data, max_time=None, label="planet", color="black", offset=0):
    try:
    ...
    plt.fill_between(keys + offset, means - half_stds, means + half_stds, alpha=0.2, color=color)
    plt.legend(loc="lower right", prop={"size": 6}).get_frame().set_edgecolor("0.1")
...
def plot_experiment(
    ...
    for exp in os.listdir(root):
        if re.match(exp_query, exp) and (
            neg_exp_query is None or re.search(neg_exp_query, exp) is None
        ):
    ...
    if slac_data is not None:
        ...
        print_planet_baseline(
            task, slac_data, max_time=max_time, label="slac", color="black", offset=offset
        )


if __name__ == "__main__":
    external_rng = np.random.default_rng(seed=1018)
    seeds = external_rng.integers(1e3, 1e8, 4)
    # root = "/workspace/repos_dev/pytorch_sac_ae/results/walker"
    root = "/workspace/repos_dev/VQVAE_RL/results/Atari/"
    # root = "/workspace/repos_dev/pytorch_sac_ae/results/gym"
    task = "Boxing-v5"
    # task = "HalfCheetah-v4"
    ...
    plot_experiment(
        ...
        root=root,
        train=False,
        value_name="avg_episodic_reward",
        key_name="timesteps_done",
        max_key=True,
        # best_k=10,
    )
