...
class MetersGroup(object):
    def __init__(self, file_name, formating=None):
    ...
    #     if self._file_name.endswith("eval.log"):
    #         print("%s: %s" % (colored("Run name", "red"), self._file_name.replace("eval.log", "")))
...
class Logger(object):
    def __init__(self, log_dir, use_tb=True, use_wandb=True, config="rl"):
...
#         self.M_train = defaultdict(AverageMeter)
#         self.M_eval = defaultdict(AverageMeter)
...
#                 self.M_eval[key].update(value)
#             else:
#                 self.M_train[key].update(value)
...
#     def dump(self, agent):  # only for traning mode
#         self.dump_counter += 1
#         if self.dump_counter % self.dump_interval == 0:
#             data = dict()
#             for key, meter in self.M_train.items():
...
class LoggerWandb:
    def __init__(self, log_dir, dump_interval=100):
        self.M_train = defaultdict(list)
        self.M_eval = defaultdict(list)
        self.log_train_path = os.path.join(log_dir, "train.log")
        self.log_train_episodic_path = os.path.join(log_dir, "train_episodic.log")
        self.log_eval_path = os.path.join(log_dir, "eval.log")
        if os.path.exists(self.log_train_path):
    ...
    def log(self, metrics: dict):
        for key, value in metrics.items():
            # self.M[key].append(value)
            if key.startswith("Evaluation"):
                self.M_eval[key].append(value)
            else:
                self.M_train[key].append(value)
    ...
    def dump(self, agent):  # only for traning mode
        self.dump_counter += 1
        if self.dump_counter % self.dump_interval == 0:
            for key, value in self.M_train.items():
                self.M_train[key] = sum(value) / len(value)
            self.M_train.update(
                {
                    "General/timesteps_done": agent.timesteps_done,
                    "General/episodes_done": agent.episodes_done,
            ...
            wandb.log(self.M_train)
            with open(self.log_train_path, "a") as f:
                f.write(json.dumps(self.M_train) + "\n")
            self.M_train = defaultdict(list)
