    ...
    def update(self, use_shaping: bool = True):
        if self.timesteps_done == self.init_steps:
            ...
            for _ in range(100):
                ...
                # X.extend([encoded, encoded_next])
            if use_shaping:
                X = self.load_states_from_memory(unique=True)
                X = self.ground_Q.encoder(X)
                # X = torch.cat(X, dim=0)
                self.kmeans.init_cluster(X)
        ...
        if self.timesteps_done % self.ground_learn_every == 0:
            for _ in range(self.ground_gradient_steps):
                ...
                (
                    ...
                    state, action, next_state, reward, terminated, use_shaping=use_shaping
                )
                if use_shaping:
                    # [update abstract_V]
                    self.update_absV_table(
                        cluster_indices, cluster_indices_next, reward, terminated, info
                    )
