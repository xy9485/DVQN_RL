...
from latentrl.common.custom_callbacks import (
    ...
    TrainingRewardWriterCallback,
    TrainingRewardWriterCallback_both,
    EpisodeCounterCallback,
    MyRewardWriterCallback,
...
def process_hparams_for_model(hparams: Dict, algo_class, env_id):
    ...
    if isinstance(hparams["learning_rate"], str) and hparams["learning_rate"].startswith("lin"):
        temp_hparams.update(
            learning_rate=linear_schedule(eval(hparams["learning_rate"].split("_")[1]))
        )
...
def train_vanilla_discrete():
    ...
    algo_class = DQN  # DQN
    policy = "CnnPolicy"
    env_id = "Boxing-v0"  # CarRacing-v0, Skiing-v0, Boxing-v0
    ...
    eval_freq = 50
    action_repetition = 2
    max_neg_rewards = None  # 100
    punishment = None  # -20

    time_tag = datetime.now().strftime("%b%d_%H-%M-%S")
    run_name = f"vanilla_discrete_{algo_class.__name__}_{time_tag}_disc_vqvae_c3_best_train"

    monitor_dir = f"/workspace/repos_dev/VQVAE_RL/log_monitor/{env_id}/{run_name}"

    always_random_start = None  # True
    no_random_start = None  # True
    ...
    hparams = {
        ...
        "tensorboard_log": f"/workspace/repos_dev/VQVAE_RL/log_tensorboard/{env_id}/",
    ...
    if not stack_mode or stack_mode == "venv_stack":
        ...
        wrapper_kwargs_list = [
            ...
            #     "no_random_start": no_random_start,
            # },
            {"vertical_cut_d": 84, "shape": 84, "num_output_channels": 3},
            {"n_frame_stack": n_stack},  # FrameStackWrapper,
    ...
    eval_env = make_vec_env_customized(
        env_id,
        1,
        seed=seed + 1,
        monitor_dir=monitor_dir,
    ...
    extra_hparams = dict(
        stack_config=f"{stack_mode}({n_stack})",
        omega="no_need",
    ...
    non_zero_reward_callback = NonZeroRewardWriterCallback(average_window_size=5)

    callback = CallbackList([hp_callback, myreward_callback, non_zero_reward_callback])
...
def train_latent2():
    ...
    eval_freq = 50
    vae_inchannel = 3
    vae_latent_dim = None  # None when using vqvae; 32 when using vae
    action_repetition = 2
    max_neg_rewards = None  # None when atari; 100 when CarRacing-v0
    punishment = None  # None when ataril; -20.0 when CarRacing-v0
    run_model = False

    time_tag = datetime.now().strftime("%b%d_%H-%M-%S")
    run_name = f"latent_{algo_class.__name__}_{time_tag}_disc_vqvae_c3"
    # vae_path = "/workspace/VQVAE_RL/logdir/beta3_vae32_channel3" #beta3_vae32_channel3
    # vae_path = "/workspace/VQVAE_RL/logdir/beta3_vae32_channel1"
    # vae_path = "/workspace/VQVAE_RL/logdir/vae"
    ...
    vae_path = f"/workspace/repos_dev/VQVAE_RL/vae_models/{env_id}/vqvae_c3_embedding16x64_3"
    ...
    hparams = {
        ...
        # linear_schedule(7.3e-4)/0.0003/4e-4(John)
    ...
    if not stack_mode or stack_mode == "venv_stack":
        ...
        wrapper_kwargs_list = [
            ...
            #     "no_random_start": no_random_start,
            # },
            {"vertical_cut_d": 84, "shape": 84, "num_output_channels": vae_inchannel},
            {
                "n_stack": n_stack,
                "vae_f": vae_path,
                "vae_sample": vae_sample,
    ...
    if run_model:
        model = DQN.load(
            "/workspace/repos_dev/VQVAE_RL/log_monitor/env_id/latent_DQN_04-25-08-44-28_disc_vqvae_c3/best_model_train.zip",
            env=env,
        )
        env = model.env
    ...
    non_zero_reward_callback = NonZeroRewardWriterCallback(average_window_size=5)
    # early_stop_callback = EarlyStopCallback(reward_threshold=0, progress_remaining_threshold=0.4)
    save_best_on_training_back = SaveOnBestTrainingRewardCallback(
        check_freq=5000, log_dir=monitor_dir
    )
    callback = CallbackList(
        [
            hp_callpack,
            myreward_callback,
            non_zero_reward_callback,
            save_best_on_training_back,
        ]
    )
    # or
    # callback = HparamsWriterCallback(run_name, hparams, extra_hparams)
    ...
    model.learn(
        ...
        eval_freq=int(total_time_steps / (n_envs * eval_freq)),
        n_eval_episodes=5,
        eval_log_path=f"/workspace/repos_dev/VQVAE_RL/log_evaluation/{run_name}",
        callback=callback,
    )
    # model.learn(total_time_steps, tb_log_name=run_name, callback=callback)
    ...
    if save_final_model:
        model.save(f"/workspace/repos_dev/VQVAE_RL/saved_final_model/{run_name}")
...
def train_shaping():
    ...
    latent_model_path = (
        "/workspace/VQVAE_RL/log_evaluation/latent_SAC_02-21-17-07-13_alwaysRS_AC/best_model"
    )
    ...
    hparams = {
        ...
        "tau": 0.02,
        "gamma": 0.98,
        # time steps for each rollout(def collect_rollouts): n_envs * train_freq
        "train_freq": 1,
        "gradient_steps": -1,
        "ent_coef": "auto",  # "auto",0.2
        "target_update_interval": 1,
    ...
    if not stack_mode:
        ...
        wrapper_kwargs_list = [
            ...
            {"vertical_cut_d": 84, "shape": 64, "num_output_channels": vae_inchannel},
            {
                ...
                "latent_dim": vae_latent_dim,
                "vae_sample": vae_sample,
                "latent_model_f": latent_model_path,
    ...
    if stack_mode == "venv_stack":
        ...
        env = VecShapeReward2(
            ...
            vae_sample=vae_sample,
            latent_deterministic=latent_deterministic,
            get_potential_mode="max",
            train=True,
        )
        eval_env = VecFrameStack(eval_env, n_stack)
        eval_env = VecShapeReward2(
            eval_env,
            vae_f=vae_path,
            latent_model_f=latent_model_path,
            vae_sample=vae_sample,
...
def train_shaping2():
    ...
    time_tag = datetime.now().strftime("%b%d_%H-%M-%S")
    run_name = f"shaping_{algo_class.__name__}_{time_tag}_disc_vqvae_c3_best_train_latent"
    # vae_path = "/workspace/VQVAE_RL/logdir/vae"
    # vae_path = "/workspace/VQVAE_RL/logdir/beta3_vae32_channel1"
    ...
    # latent_model_path = (
    #     "/workspace/VQVAE_RL/saved_final_model/latent_DQN_03-08-19-44-57_discrete.zip"
    # )
    # latent_model_path = "/workspace/VQVAE_RL/saved_final_model/latent_DQN_04-06-19-11-05_discrete_vq_c1.zip"

    latent_model_path = "/workspace/repos_dev/VQVAE_RL/log_monitor/env_id/latent_DQN_04-25-08-44-28_disc_vqvae_c3/best_model_train.zip"
    # latent_model_path = "/workspace/repos_dev/VQVAE_RL/saved_final_model/latent_DQN_04-25-08-44-28_disc_vqvae_c3.zip"
    # latent_model_path = "/workspace/repos_dev/VQVAE_RL/log_evaluation/latent_DQN_04-25-08-44-28_disc_vqvae_c3/best_model.zip"
...
def run_model():
    ...
    hparams = {
        # linear_schedule(7.3e-4), 0.0003, 0.0004(John)
    ...
    model = DQN.load(
        "/workspace/VQVAE_RL/log_evaluation/vanilla_discrete_DQN_03-05-16-31-37/best_model.zip",
        env=env,
    )
    # env = model.env
    # Enjoy trained agent
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--logdir",
        type=str,
        default="/workspace/VQVAE_RL/logdir/",
    ...
    # wandb.init(project="Cascading_RL", sync_tensorboard=True)
