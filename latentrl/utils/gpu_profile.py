    ...
    if event == "line":
        try:
            ...
            if lineno is not None:
                ...
                mem_display = (
                    new_meminfo_used - last_meminfo_used if use_incremental else new_meminfo_used
                )
                with open(gpu_profile_fn, "a+") as f:
                    f.write(
                        f"{where_str:<50}" f":{(mem_display)/1024**2:<7.1f}Mb " f"{line.rstrip()}\n"
                    )
                    ...
                    if print_tensor_sizes is True:
                        ...
                        new_tensor_sizes = {
                            (type(x), tuple(x.size()), x.dbg_alloc_where) for x in get_tensors()
                        }
