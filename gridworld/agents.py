...
from utils import (
    AlphaDecayScheduler,
    CountBasedAlphaScheduler,
    CountBasedEpsilonScheduler,
    EpsilonDecayScheduler,
    Scheduler,
    TemperatureDecayScheduler,
    softmax,
)


class Agent:
    def __init__(self, alpha: float, gamma:float, action_space, observation_space, random_value_init):
        ...
        self.alpha_scheduler: AlphaDecayScheduler | CountBasedAlphaScheduler = None  # this attribute is given from outside after instantiating the agent
        self.explore_scheduler: Scheduler = None  # this attribute is given from outside after instantiating the agent
    ...
    def choose_action(self, state, Q=None, greedy=False):
        ...
        if greedy:
            ...
            return int(action), info_dict
        if isinstance(self.explore_scheduler, TemperatureDecayScheduler):
            # compute softmax probability given self.Q[state, :]
            prob = softmax(Q[state, :], temperature=self.explore_scheduler.get_value())
        ...
        if isinstance(self.explore_scheduler, EpsilonDecayScheduler):
            eps = self.explore_scheduler.get_value()
        elif isinstance(self.explore_scheduler, CountBasedEpsilonScheduler):
            eps = self.explore_scheduler.get_value(state)
    ...
    def compute_action_prob(self, state, action, Q, softmax_temprature, eps):
        # assert temprature or eps won't be true at the same time
        # assert not (temprature and eps)
        if softmax_temprature:
            prob = self.softmax(Q[state, :], temperature=softmax_temprature)
...
class Q_Agent(Agent):
    def __init__(self, alpha, gamma, action_space, observation_space, random_value_init):
        super().__init__(alpha, gamma, action_space, observation_space, random_value_init)
        self.reset_Q(self.random_value_init)

    def learn(self, state, action, reward, state_, done):
        # check if done is a numpy array
        alpha = self.alpha_scheduler.get_value(state[0], action[0]) if isinstance(self.alpha_scheduler, CountBasedAlphaScheduler) else self.alpha_scheduler.get_value()
        Q_error = reward + self.gamma * np.max(self.Q[state_, :]) * (1 - done) - self.Q[state, action]
        self.Q[state, action] = self.Q[state, action] + alpha * Q_error
        return {
...
class DoubleQ_Agent(Agent):
    ...
    def learn(self, state, action, reward, state_, done):
        ...
        if np.random.random() > 0.5:
            ...
            self.Q[state, action] += alpha * Q_error
        else:
            ...
            self.Q_[state, action] += alpha * Q_error
...
class VQ_Agent(Agent):
    def __init__(self, alpha, gamma, action_space, observation_space, random_value_init, importance_sampling=False):
        super().__init__(alpha, gamma, action_space, observation_space, random_value_init)
        # self.alpha_v = alpha
        # self.alpha_q = alpha
    ...
    def learn_importance_sampling(self, state, action, reward, state_, done, prob_action):
        ...
        assert isinstance(self.explore_scheduler, TemperatureDecayScheduler), (
            "importance sampling only works with softmax exploration"
        )
        action_prob_current_pi = self.compute_action_prob(
            state, action, self.Q, softmax_temprature=self.explore_scheduler.get_value(), eps=self.eps
        )
    ...
    def learn(self, state, action, reward, state_, done):
        ...
        self.V[state] = self.V[state] + alpha * V_error
        ...
        return {
            ...
            # "Q(s_,a_)": np.mean(self.Q[state_, action_]) if not done else 0,
            "alpha": alpha,
            "eps": self.explore_scheduler.get_value(state[0]) if isinstance(self.explore_scheduler, CountBasedEpsilonScheduler) else self.explore_scheduler.get_value(),
        }
    ...
    def learn_syncVQ(self, state, action, reward, state_, done, prob_action):
        ...
        if self.importance_sampling:
            ...
            bool_mask = action == max_action
            bool_mask = bool_mask.astype(float)
            current_action_prob = (1 - self.eps + self.eps / self.n_actions) * bool_mask + (
                self.eps / self.n_actions
            ) * (1 - bool_mask)
            # importance sampling ratio
...
class SARSA_Agent(Agent):
    ...
    def learn_batch(self, state, action, reward, state_, done):
        assert state.shape == action.shape == state_.shape == done.shape == reward.shape

        alpha = self.alpha_scheduler.get_value(state, action) if isinstance(self.alpha_scheduler, CountBasedAlphaScheduler) else self.alpha_scheduler.get_value()

        # state_ is a numpy array, self.choose_action only take a single state as interger as input, therefore we need to handle each state in state_ separately
        action_, _ = zip(*[self.choose_action(int(s)) for s in state_])
        action_ = np.array(action_, dtype=int) # not necessary, but for clarity and consistency

        assert action_.shape == state_.shape, "state_ and action_ must have the same shape"
        # since action_.shape == state_.shape, no need to use np.ix_
    ...
    def learn(self, state, action, reward, state_, done):
        alpha = self.alpha_scheduler.get_value(state[0], action[0]) if isinstance(self.alpha_scheduler, CountBasedAlphaScheduler) else self.alpha_scheduler.get_value()
        action_, _ = self.choose_action(state_[0])
        Q_error = reward + self.gamma * self.Q[state_, action_] * (1 - done) - self.Q[state, action]
        self.Q[state, action] = self.Q[state, action] + alpha * Q_error
        return {
            "Q_error": np.mean(Q_error),
...
class ExpectedSARSA_Agent(Agent):
    ...
    def learn_batch(self, state, action, reward, state_, done):
        ...
        q_values_sum = np.sum(q_values, axis=1, keepdims=False)
        q_values_max = np.max(q_values, axis=1, keepdims=False)
        q_values_max_count = np.sum(q_values == q_values_max[:, np.newaxis], axis=1, keepdims=False)

        if isinstance(self.explore_scheduler, CountBasedEpsilonScheduler):
            eps = np.array([self.explore_scheduler.get_value(s) for s in state_])
        elif isinstance(self.explore_scheduler, EpsilonDecayScheduler):
            eps = self.explore_scheduler.get_value()
        elif isinstance(self.explore_scheduler, TemperatureDecayScheduler):
            raise ValueError("ExpectedSARSA_Agent does not support TemperatureDecayScheduler for now")

        assert eps.shape == q_values_max_count.shape == q_values_max.shape, "eps and state_ must have the same shape"
        expected_value_for_max = (
            q_values_max * ((1 - eps) / q_values_max_count + eps / self.n_actions) * q_values_max_count
        )
        expected_value_for_non_max = (q_values_sum - q_values_max * q_values_max_count) * (eps / self.n_actions)
    ...
    def learn(self, state, action, reward, state_, done):
        alpha = self.alpha_scheduler.get_value(state[0], action[0]) if isinstance(self.alpha_scheduler, CountBasedAlphaScheduler) else self.alpha_scheduler.get_value()
        if isinstance(self.explore_scheduler, CountBasedEpsilonScheduler):
            eps = self.explore_scheduler.get_value(state_[0])
        elif isinstance(self.explore_scheduler, EpsilonDecayScheduler):
            eps = self.explore_scheduler.get_value()
        elif isinstance(self.explore_scheduler, TemperatureDecayScheduler):
            raise ValueError("ExpectedSARSA_Agent does not support TemperatureDecayScheduler for now")
...
def get_agent(config, env):
    if config["algo_name"] == "VQ-learning":
        agent = VQ_Agent(
            ...
            random_value_init=config["random_value_init"],
